---
name: sync-upstream
description: 同步上游 kiali/kiali-operator 指定 tag 到本仓库（alauda-mesh/build-kiali）。完成：复制上游源码并裁剪版本（roles 只保留最新 3 个版本 + default）、更新 VERSION/Makefile/CSV/CRD、生成新版本 wolfi 与 apko 构建配置、更新三条 GitHub 流水线、创建 PR 并监控流水线（失败时分析原因并修复）。仅限用户显式通过 /sync-upstream 调用。
...
- kiali server（kiali/kiali）与 operator（kiali/kiali-operator）的 tag 版本号保持一致，wolfi 构建的 `expected-commit` 取的是 **kiali/kiali** 仓库 tag 的 commit。
- 脚本间通过 `_output/sync-state.env` 传递状态（KEEP/DROP 版本列表等），`_output/` 已在 gitignore 中。
- 全程禁止 `git commit --amend`，一律创建新 commit。步骤 6 之前不要 push、不要建 PR。
...
脚本会自动：前置检查（工作区干净、tag 格式）→ 基于 `origin/<目标分支>` 创建 `sync/<tag>` 分支 → 浅克隆上游到 `_output/upstream/` → 解析 kiali/kiali 的 expected-commit → 计算保留/移除版本 → 复制 roles/playbooks/watches → 裁剪 roles 与 kiali-default-supported-images.yml → 更新 VERSION → 复制上游 CRD → 自动提交一个 commit。
...
- wolfi/configs：以最新版本文件为模板生成新版本的 `wolfi/kiali-<版本>.yaml`（改 name/version/epoch=0/expected-commit）和 `configs/kiali/v<版本>.apko.yaml`，删除被淘汰版本的文件；
...
把步骤 2～4 的全部修改提交为一个新 commit（如 `chore: bump kiali to <tag>`；若 CSV 语义合并改动较大可单独一个 commit）。然后向用户汇报（这是用户 review 的依据）：

1. 同步分支名、保留/移除的版本列表、是否有 RESTORED 恢复项；
2. Makefile/CSV/流水线的版本变更概要，`OPERATOR_BASE_IMAGE_VERSION` 的新值；
3. wolfi 构建步骤审阅结论（是否沿用模板、有无工具链调整）；
4. CSV 上游 diff 逐条结论（合入了什么 / 为什么跳过）；
5. check-leftovers 的 WARN 项说明。

## 步骤 6：创建 PR 并监控三条流水线

先把 PR 描述写进临时文件（scratchpad 下，如 `pr-body.md`）：内容取步骤 5 汇报的精简版，结尾加一行 `🤖 Generated with [Claude Code](https://claude.com/claude-code)`。然后：
